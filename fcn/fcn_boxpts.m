function [f,hdl,pts] = fcn_boxpts(x,lab,cmap,samenum,w,f)
% clear all
% close all
% clc
...
for j = 1:length(u)
    ...
    if samenum ~= 0
        r = randperm(length(vals),min(length(vals),samenum));
        vals = vals(r);
    end
    ...
    hdl.boxhandle{j} = plot(xvals,yvals,'color',cmapdark(j,:),'linewidth',0.75);
    hdl.medianhandle{j} = plot(j + [-wid,wid],prct(2)*ones(1,2),'color',cmapdark(j,:),'linewidth',0.75);
    
    
    xvals = [j*ones(1,2), nan, j*ones(1,2)];
    yvals = [prct(1),whisk1,nan,prct(3),whisk2];
    hdl.whiskerhandle{j} = plot(xvals,yvals,'color',cmapdark(j,:),'linewidth',0.75);
